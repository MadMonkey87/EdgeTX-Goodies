...
---- #                                                                       #
---- # This program is free software; you can COLOR_THEME_ACTIVEistribute it and/or modify  #
...
-- customized by Philippe Wechsler

local version = "v0.6 customized"
...
-- color for battery
-- This function returns green at 100%, COLOR_THEME_ACTIVE bellow 30% and graduate in between
...
-- color for cell
-- This function returns green at gvalue, red_value at rvalue and graduate in between
...
local function refreshZoneTiny(wgt)
  ...
  lcd.drawText(wgt.zone.x + wgt.zone.w - 25, wgt.zone.y + 5, wgt.cellPercent .. "%", RIGHT + SMLSIZE + CUSTOM_COLOR + wgt.shadowed + wgt.no_telem_blink)
  lcd.drawText(wgt.zone.x + wgt.zone.w - 25, wgt.zone.y + 20, myString, RIGHT + SMLSIZE + CUSTOM_COLOR + wgt.shadowed + wgt.no_telem_blink)
  -- draw batt
  local rect_h = math.floor(25 * wgt.cellPercent / 100)
  lcd.drawFilledRectangle(wgt.zone.x + 50, wgt.zone.y + 9 + 25 - rect_h, 16, rect_h, getPercentColor(wgt.cellPercent) + wgt.no_telem_blink)
  lcd.drawRectangle(wgt.zone.x + 50, wgt.zone.y + 9, 16, 25, CUSTOM_COLOR, 2)
  lcd.drawFilledRectangle(wgt.zone.x + 50 + 4, wgt.zone.y + 7, 8, 3, CUSTOM_COLOR)
end

--- Zone size: 160x32 1/8th
local function refreshZoneSmall(wgt)
  local myBatt = { ["x"] = 0, ["y"] = 0, ["w"] = 155, ["h"] = 35, ["segments_w"] = 25, ["color"] = BLUE, ["cath_w"] = 6, ["cath_h"] = 20 }

  -- draws bat
  lcd.setColor(CUSTOM_COLOR, COLOR_THEME_ACTIVE)
...
local function refreshZoneMedium(wgt)
  local myBatt = { ["x"] = 0, ["y"] = 0, ["w"] = 85, ["h"] = 35, ["segments_w"] = 15, ["color"] = COLOR_THEME_ACTIVE, ["cath_w"] = 6, ["cath_h"] = 20 }
  ...
  if wgt.zone.h > 80 then
    ...
    lcd.drawText(wgt.zone.x, wgt.zone.y + 70, string.format("dV %2.2fV", wgt.cellMax - wgt.cellMin), SMLSIZE + CUSTOM_COLOR + wgt.shadowed +  wgt.no_telem_blink)
    lcd.drawText(wgt.zone.x, wgt.zone.y + 84, string.format("Min %2.2fV", wgt.cellDataHistoryCellLowest), SMLSIZE + CUSTOM_COLOR + wgt.shadowed + wgt.no_telem_blink)
  end
  ...
  for i = 1, wgt.cellCount, 1 do
    ...
    lcd.setColor(CUSTOM_COLOR, COLOR_THEME_SECONDARY2)
    lcd.drawFilledRectangle(wgt.zone.x + cellX + (cellW * wgt.cellDataHistoryLowestPercent[i])/100 -2, cellY, 2 , cellH, CUSTOM_COLOR)

    lcd.setColor(CUSTOM_COLOR, COLOR_THEME_ACTIVE)
    lcd.drawText           (wgt.zone.x + cellX + 10, cellY, string.format("%.2f", wgt.cellDataLive[i]), SMLSIZE + COLOR_THEME_PRIMARY1 + wgt.no_telem_blink)
  ...
  lcd.drawText(wgt.zone.x + myBatt.x + 20, wgt.zone.y + myBatt.y + 4, string.format("%2.0f%%", wgt.cellPercent), LEFT + MIDSIZE + COLOR_THEME_PRIMARY1)

end

--- Zone size: 192x152 1/2
local function refreshZoneLarge(wgt)
  local myBatt = { ["x"] = 0, ["y"] = 18, ["w"] = 76, ["h"] = 121, ["segments_h"] = 30, ["color"] = COLOR_THEME_ACTIVE, ["cath_w"] = 30, ["cath_h"] = 10 }
  ...
  for i = 1, wgt.cellCount, 1 do
    ...
    lcd.setColor(CUSTOM_COLOR, COLOR_THEME_ACTIVE)
    lcd.drawText(wgt.zone.x + pos[i].x + 10, wgt.zone.y + pos[i].y, string.format("%.2f", wgt.cellDataLive[i]), COLOR_THEME_PRIMARY1)
...
local function refreshFullScreenImpl(wgt, x, w, y, h)
  ...
  local myBatt = { ["x"] = 10, ["y"] = 20, ["w"] = 80, ["h"] = 121, ["segments_h"] = 30, ["color"] = CUSTOM_COLOR, ["cath_w"] = 30, ["cath_h"] = 10 }
  ...
  for i = 1, wgt.cellCount, 1 do
    ...
    lcd.setColor(CUSTOM_COLOR, COLOR_THEME_ACTIVE)
    lcd.drawText(x + pos[i].x + 10, y + pos[i].y, string.format("%.2f", wgt.cellDataLive[i]), COLOR_THEME_PRIMARY1 + wgt.no_telem_blink)
  ...
  for i = 1, wgt.cellCount, 1 do
    ...
    lcd.setColor(CUSTOM_COLOR, COLOR_THEME_ACTIVE)
    lcd.drawRectangle(x + pos[i].x, y + pos[i].y, 54, 20, CUSTOM_COLOR, 1)
    lcd.drawText(x + pos[i].x + 10, y + pos[i].y, string.format("%.2f", wgt.cellDataHistoryLowest[i]), COLOR_THEME_PRIMARY1 + wgt.no_telem_blink)
  end
...
local function refreshFullScreen(wgt, event, touchState)
  ...
  local y = 0
  local h = 252
  refreshFullScreenImpl(wgt, x, w, y + 40, h)
  lcd.drawFilledRectangle(0, 0, LCD_W, 40, COLOR_THEME_SECONDARY1)
  lcd.drawText(10, 40 / 2 - 2, "Battery",VCENTER + DBLSIZE + COLOR_THEME_PRIMARY2)
end
